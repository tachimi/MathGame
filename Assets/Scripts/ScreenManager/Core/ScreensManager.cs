...
namespace ScreenManager.Core
{
    public class ScreensManager : BaseScreenManager
    {
        ...
        protected override void OnEnable()
        {
            ...
            _subscriptions.Add(EventStreams.Game.Subscribe<ScreenSetActiveException>(HandleScreenSetActiveException));
        }
            ...
            where TScreen : IScreenWithTypedContext<TContext>
        {
            ...
            var enqueueScreenEvent = EnqueueScreenEvent.Create(screenTypeName.GetHashCode(), shouldHidePrevious,
                isBlockingQueue, showLoadingIndicator);
            ...
            enqueueScreenEvent.Publish(EventStreams.Game);
        }
        ...
        public static void EnqueueScreen<TScreen>(IScreenCondition showCondition = null,
            IScreenCondition expireCondition = null,
        ...
        public static void EnqueueScreen<TScreen, TContext>(TContext context, Func<bool> showCondition,
            Func<bool> expireCondition = null,
            bool shouldHidePrevious = false, bool isBlockingQueue = true, int priority = 0)
            where TScreen : IScreenWithTypedContext<TContext>
        {
            var screenTypeName = typeof(TScreen).Name;
            var enqueueScreenEvent =
                EnqueueScreenEvent.Create(screenTypeName.GetHashCode(), shouldHidePrevious, isBlockingQueue);
            enqueueScreenEvent.AddScreenContext(context);
        ...
        public static void OpenScreen<TScreen, TContext>(TContext context, bool shouldHidePrevious = false,
        ...
        public static void OpenScreen<TScreen>(bool shouldHidePrevious = false, bool showLoadingIndicator = true)
        ...
        {
            BackToPreviousScreenEvent.Create().Publish(EventStreams.Game);
        }

        public static ScreenId? GetTopScreen()
        {
            var getTopScreenEvent = new GetTopScreenEvent();
            getTopScreenEvent.Publish(EventStreams.Game);
        ...
        public static void CloseAllScreens()
        {
            ...
            closeAllScreens.Publish(EventStreams.Game);
        }
