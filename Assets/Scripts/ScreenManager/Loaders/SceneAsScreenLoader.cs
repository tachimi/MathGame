...
namespace ScreenManager.Loaders
{
    public class SceneAsScreenLoader : IScreenLoader
    {
        ...
        private readonly Dictionary<ScreenId, Action<float>> _setLoadingProgressCallbacks =
            new Dictionary<ScreenId, Action<float>>();
        ...
        {
            _sceneLoadRequests.Enqueue(new SceneLoadRequest()
                { Type = type, ShowLoadingIndicator = showLoadingIndicator });
        ...
        private void LoadNextScene()
        {
            ...
            if (UnityEngine.SceneManagement.SceneManager.GetSceneByName(loadingInformation.Path).isLoaded)
            {
                OnSceneLoaded(UnityEngine.SceneManagement.SceneManager.GetSceneByName(loadingInformation.Path), type,
                    loadingInformation);
                return;
            }
        ...
        private IEnumerator LoadScene(string sceneName, Action<Scene> onSceneLoadedSuccessfully,
            Action<string> onLoadSceneFailed)
        {
            yield return UnityEngine.SceneManagement.SceneManager.LoadSceneAsync(sceneName, LoadSceneMode.Additive);

            onSceneLoadedSuccessfully?.Invoke(UnityEngine.SceneManagement.SceneManager.GetSceneByName(sceneName));
        }
        ...
        private void OnFailSceneLoading(string message, ScreenId type, SceneScreenSettings settings)
        {
            ...
            new FailScreenLoadingEvent(type).Publish(EventStreams.Game);
        ...
        private void OnSceneLoaded(Scene scene, ScreenId type, SceneScreenSettings settings)
        {
            ...
            ScreenLoadedEvent.Create(sceneScreen, type).Publish(EventStreams.Game);
        ...
        public AsyncOperation Unload(IScreen screen)
        {
            ...
            var scene = UnityEngine.SceneManagement.SceneManager.GetSceneByName(path);
            if (!scene.isLoaded)
            {
                return null;
            }

            var result = UnityEngine.SceneManagement.SceneManager.UnloadSceneAsync(scene);
