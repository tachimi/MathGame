...
public class AdvancedDropdown : MonoBehaviour{
    ...
    public void ChangeState() {
        // Проверяем анимацию перед изменением состояния
        bool animationInProgress = false;

        switch (dropdownAnimationType) {
            case AnimationDropdownTypes.Shrinking:
                animationInProgress = ratioShrinking < 0.99f && ratioShrinking > 0.01f;
                break;
            case AnimationDropdownTypes.Fading:
                animationInProgress = ratioFading < 0.99f && ratioFading > 0.01f;
                break;
            case AnimationDropdownTypes.ShrinkingAndFading:
                animationInProgress = (ratioFading < 0.99f && ratioFading > 0.01f) || (ratioShrinking < 0.99f && ratioShrinking > 0.01f);
                break;
        }
    ...
    public void OpenOptions() {
        ...
        foreach (var option in spawnedList) {
        ...
        // Очищаем контент от возможных оставшихся элементов
        for (int i = optionsContent.childCount - 1; i >= 0; i--) {
            var child = optionsContent.GetChild(i);
            if (child != null && child.gameObject != firstObj) {
